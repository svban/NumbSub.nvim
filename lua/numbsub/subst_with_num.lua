...
local RegexUtil = require "subst_with_num.regex_util"
...
function M.subst_with_num(args)
    ...
    if mode == "ma" then
        if pattern == nil or step_value == nil then
            print "Invalid arguments. Usage for mode 'ma': :NumbSub p<pattern> S<step> [w<width>|W<width>|w|W] m<a> [c]"
            return
        end
        ...
        if start == nil or pattern == nil or n == nil or step_value == nil or mode == nil then
            print "Invalid arguments. Usage: :NumbSub s<start> p<pattern> n<count> S<step> [w<width>|W<width>|w|W] m<s|a|p|r|R> [c]"
            return
        end
    end
